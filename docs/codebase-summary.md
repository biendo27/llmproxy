...
- `config/cliproxyapi-local-config.yaml`: Optional local CLIProxyAPI config generated by setup.
...
### 2) Setup Wizard
- `src/llmproxy-install-and-setup.zsh` drives the setup flow (deps check, binary install, env creation).
- Optional config generation uses `src/llmproxy-setup-prompts-and-cliproxyapi-config-generator.zsh` to write `config/cliproxyapi-local-config.yaml`.
- Setup updates the local profile URL/key, run mode, default proxy mode, and preset in `config/llmproxy.env`.

### 3) Apply Proxy Environment
...
- This summary reflects the current repo layout.
